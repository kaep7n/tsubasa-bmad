...
describe('TeamService', () => {
        ...
        insert: jasmine.createSpy('insert').and.returnValue({
          select: jasmine.createSpy('select').and.returnValue({
            single: jasmine.createSpy('single').and.returnValue(
              Promise.resolve({ data: mockTeamData, error: null })
            )
          })
        }),
        select: jasmine.createSpy('select').and.returnValue({
      ...
      storage: {
        from: jasmine.createSpy('storageFrom').and.returnValue({
          upload: jasmine.createSpy('upload').and.returnValue(
            Promise.resolve({ data: { path: 'team-123.png' }, error: null })
          ),
  ...
  describe('createTeam', () => {
    ...
    it('should handle database error', (done) => {
      ...
      insertSpy().select().single.and.returnValue(
  ...
  describe('getUserTeam', () => {
    ...
    it('should return null when no team found', (done) => {
      ...
      selectSpy().eq().single.and.returnValue(
